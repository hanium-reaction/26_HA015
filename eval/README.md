...
| **4′** | **D4 `first_step` 의 길이·숫자는 해소, 영문은 미해소** | 길이(easy `[10,14]`→`[9,12]`, boundary 와 겹침)와 숫자는 3차 재의뢰로 사라졌다. 그러나 이 행이 함께 지목한 **"영단어 포함"** 은 남아 있다 — `d4-boundary-cert` 의 `SQL 기출문제집 펼치기`. 검사기에 `first_step_has_latin` 이 없어서 안 보였을 뿐이다. 「M27·M28 을 읽을 때」 참조 |
| **5′** | **`d1-*-portfolio` 의 easy/boundary 차이는 해소, 형제 대비 이탈은 남음** | 두 op 가 모두 `after: l2`·60분이 되어 **서로 다르다**는 tell 은 사라졌다. 그러나 이 행이 함께 적은 "형제 카드가 전부 120분인데 거기서 벗어난다" 는 그대로다 — 오히려 양쪽 다 이탈해 **M28 정답 키가 `argmin(분량)` 으로 균일하게 노출**된다 |
...
| **10** | **D1 에 대해서는 채점 기준 힌트를 의도적으로 흘렸다** | 재의뢰 시 "다시/재-/복습/회독/한 번 더 를 쓰지 말라" 는 제약을 줬다. 감춘 루브릭의 D1 정상 앵커가 회독·복습을 중복이 아니라고 규정하므로 **이것은 힌트가 맞다.** 이 힌트 없이는 같은 자리에 세 번째로 빠질 것이 확실해 전달했다. **held-out 이 순수하다고 주장하지 않는다** — M27(D1)은 이 단서를 달아 보고한다 |
...
> **이 절이 보고서 「한계」의 핵심이다.** M27(결함 탐지)·M28(주입 지점 지목)을 유형별로
...
|---|---|---|
| **D1** | `first_step` 영문 포함(portfolio 쌍) | 완전 분리자 1개. 값 자체는 우연일 수 있으나 **배제할 수 없다** |
| **D2** | **`마저`** 가 easy 두 계획에만. 무결함 대조군 12건 발화 **0건** | **가장 나쁘다.** "계획에 `마저` 가 있으면 D2 결함" 이라는 한 줄 규칙이 오탐 없이 2/2 를 맞힌다. **M27(D2)를 검토기의 의미 판단으로 읽으면 안 된다** |
| **D3** | 타깃 위치만 | 어휘·문구 분리자 없음. **가장 깨끗하다** |
| **D4** | 비채움 `first_step` 이 `하기` 로 끝나는 것이 easy 에만 + `SQL` 영문 | 대조군 3건에서도 발화해 규칙으로는 시끄럽지만, 유형 안에서는 완전 분리 |
| **D5** | **`사이트`** 가 boundary 두 계획에만. 대조군 0건 | D2 와 같은 구조 |
| **전 유형** | 타깃 노드의 **목록 위치**가 easy/boundary 를 가른다(D2·D3·D4) | `after_anchor` 는 `insert_item` 에만 걸려 있어 `replace_*` 8건이 위치 검사 밖이었다 |

**M28 은 별도로 더 약하다.** `insert_item` 8건 중 **6건**에서 심은 카드가 계획의 유일한
비최빈 분량이라 `argmin(분량)` 하나로 정답 키가 나온다(`d1-*-portfolio`, `d5-*` 4건).
M28 을 "검토기가 결함 위치를 짚었다" 로 읽을 수 있는 것은 나머지 2건뿐이다.
