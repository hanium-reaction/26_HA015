...
| 4 | **`d5-easy` 2건의 `category: "life"`** | `goal_decompose.v2.md` 어휘에 없는 값이라 `_normalize_category` 가 `other` 로 바꾼다. 66건 중 계획 카테고리가 섞인 건 이 둘뿐이고 `d5-boundary` 는 목표 카테고리를 그대로 쓴다 — 즉 `category` 하나로 D5 의 easy/boundary 가 **100%** 갈린다. M27(D5) 는 부풀려지는 정도가 아니라 **해석 불가** |
| 5 | **`d1-easy-cert` 가 루브릭 D1 정상 앵커와 모순** | 심은 것은 "SQL 이론 **복습**하기"(`approved=false`)인데, 루브릭 D1 정상 행은 "회독·**복습**·심화는 중복이 아니다"라고 쓴다. 경계 행에도 걸린다. 루브릭이 검토기 프롬프트의 본문이므로 **루브릭을 충실히 따르는 검토기는 이 케이스를 통과시킨다** — M27(D1-easy)의 상한이 1/2 이 된다 |
