...
| ~~4′~~ | ~~D4 `first_step` 길이 tell~~ | ✅ **2026-09-02 해소.** 아래 **전면 재의뢰**로 함께 처리됐다 |
| ~~5′~~ | ~~`d1-*-portfolio` 위치·분량 tell~~ | ✅ **2026-09-02 해소.** 〃 |
| **9** | **기계적 게이트가 내용 품질을 깎았다 (이번 접근의 비용)** | 지름길 14건을 없애려고 결함 20건 중 10건을 재의뢰했는데, 통과 판정을 **사람이 아니라 `scripts/check_seeded_defect_shortcuts.py`** 가 하게 한 결과 세 곳에서 내용이 나빠졌다. ① D1 boundary-portfolio 는 앵커 일치 규칙 때문에 **시나리오가 통째로 바뀌었다** — 원래(배포 후 교차 브라우저 확인)가 더 선명한 경계였다. ② D2 boundary-cert 가 영문·숫자 신호를 피하려 "SQL 활용 단원"→"쿼리 단원" 이 되어 **계획 자신의 어휘와 어긋난다**. ③ D4 boundary 2건이 길이·숫자를 줄이려 "문제집 **1페이지** 펼치기"→"문제집 펼치기" 로 깎여 **"통과가 정답" 이라는 성질이 조금 약해졌다.** 셋 다 작성자가 스스로 보고했고(숨기지 말라고 지시했다), 숨겨진 왜곡보다 기록된 왜곡이 낫다고 판단해 채택했다. 자세한 것은 `first_plan_seeded_defects.json` 의 `provenance.reauthored_2026_09_02` |
