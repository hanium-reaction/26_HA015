...
> | **M22** | `마지막 계획일 ≥ min(마감, 지평 상한)` (**슬랙 없음**) | **선택** — `horizon_coverage_notice` 를 쓰지 않는다(위 오용). 덮어야 할 날을 **지평 상한으로 자른** 뒤 비교한다. 프로덕션의 3일 슬랙도 안 쓴다 |
> | **M23** | `missing_milestone_titles` **빈 목록** | **유도** + 카브아웃 |
> | **M24** | `drop_out_of_cycle_branches` 가 걷어낸 branch **0건** | **유도** + 카브아웃 |
> | **M25** | `_WAITING_TITLE_RE` 백스톱이 잡은 leaf **0건** | **유도** — 〃 |
>
> **정직한 셈: 유도 6 · 선택 2 · 미정 1.** 초판의 "8" 은 과장이었다.
>
> #### M18 을 미정으로 둔다 — 이것이 이 정정의 결론이다
>
> 초판의 반-세션 규칙은 전제가 거짓이다. M18 은 **③층 보정 전 원안**에서 재는데, 원안은
> LLM 이 만든 임의의 `estimated_minutes` 다. "계획은 세션 단위로만 만들어진다" 가 성립하지
> 않는다 — 애초에 그게 성립하면 M17 이 자명하게 100% 다. 보정 후에도 균일하지 않다:
> `normalize_action_minutes` 는 **클램프만** 하고, ADR-0009 D2 가 *"평균을 상한으로 쓰면
> 길이가 내용을 따라갈 수 없다"* 며 **일부러 입도를 없앴다.**
>
> ⚠️ **그리고 M19 가 M18 의 위쪽을 이미 막는다.** `_take_within_budget` 은 누적 분이 예산을
> 1분이라도 넘으면 자르므로, 예산 초과 계획은 **반드시 M19 에서 탈락**한다. 즉 M26 수준에서
> M18 의 실효 밴드는 **`[예산 − x, 예산]` 편측**이고, §5 M18 정의의 "양방향" 은 M26 안에서는
> 성립하지 않는다. 이 상호작용을 모르고 밴드를 정하면 안 된다.
>
> **그래서 M18 의 임계값은 팀이 정한다.** 남은 선택지는 셋이고 전부 자의가 들어간다:
> ① 고정 비율(±X%) ② 예산 이하만 요구하는 편측 하한 ③ M18 을 M26 의 AND 에서 빼고
> 별도 보고. **M26 은 이것이 정해지기 전에는 나오지 않는다.**
>
> #### 계산 불가 처리 — **지표를 빼지, 케이스를 빼지 않는다**
>
> 초판은 *"한 지표라도 계산 불가인 케이스는 M26 분모에서 뺀다"* 고 적었는데, 그대로 적용하면
> **M26 분모가 34 → 6 이 된다**(마일스톤 케이스만 남는다). 그건 M26 을 무의미하게 만든다.
>
> **규칙을 바꾼다 — M26 은 그 케이스에 대해 정의된 지표들의 AND 이고, 케이스마다 몇 개의
> 지표가 걸렸는지 함께 보고한다.** 지표 수가 다른 케이스를 한 비율로 뭉개지 않도록,
> **정의된 지표 수별로 나눠서도 낸다.**
>
> | 지표 | 정의되는 범위 | 골든셋 |
> |---|---|---|
> | **M22** | 마감이 지평 상한 **안**에 있는 케이스 | 11/34 — 나머지 23건은 시스템이 **의도적으로** 안 덮는다 |
> | **M23** | 이번 주기 확정 마일스톤이 있는 케이스 | **6/34** |
> | **M24** | 창(`cycle_milestone_window`)이 남은 마일스톤 **전부를 덮지 않는** 케이스 | 하네스의 `m24_measurable` 조건 |
>
...
> §2.3 이 M24 분모를 42 → 24 로 줄인 실제 조건은 **창 커버리지**였다. 자기 선례를 잘못
> 인용했던 것을 고친다.
>
> #### 반복 3회 집계 — **초판에 없던 자유도**
>
> §5 의 M26 분모는 "케이스 수"(34)인데 실행은 34 × 3 = **102 계획**을 낸다. 접는 방법을
> 안 정하면 M26 이 크게 움직인다 — [`l1-7-results.md`](l1-7-results.md) §4 가
> **"34개 중 9개가 회차마다 M17 통과/실패가 갈린다"** 고 적었다.
>
> **등록: 1차 추정은 `PRIMARY_REPEAT = 0` 의 고유 34건.** L1-7B 가 M29 에 쓰는 규칙과 같다 —
> 반복을 독립 표본으로 세면 신뢰구간이 거짓으로 좁아진다. 회차별 M26 세 값과 "3회 모두
> 통과" 비율은 **보조로 함께** 낸다(그건 M26 이 아니다).
>
> #### M26 을 내려면 아직 남은 것
>
> | # | 막는 것 | 왜 |
> |---|---|---|
> | 1 | **M18 임계값 결정** | 위 세 선택지 중 하나를 팀이 골라야 한다 |
> | 2 | **L1-7A v2 재실행** | 현재 M18 수치는 v1 슬롯 산출이다 |
> | 3 | **스케줄러 경로를 하네스에** | M20·M21·M22 가 배치 결과를 요구한다. 현재 하네스가 계산하는 것은 **M17·M18·M19·M23·M24·M25 여섯 개**뿐이다 |
>
> **전부 되기 전에는 M26 을 내지 않는다.** 부분 집합의 AND 를 M26 이라 부르지 않는다.
>
> #### M33 과의 관계
>
> **M33 = ΔM26 이므로 위 세 개를 그대로 이어받는다.** 추가로:
>
> - **arm 은 셋이다** (④층 제거 · 피드백 비운 재분해 · 정상). 초판은 "두 arm" 이라 적었다.
> - **같은 기준만으로 부족하고 같은 분모가 필요하다.** 어떤 케이스가 한 arm 에서만 M26
>   분모에서 빠지면 ΔM26 이 **서로 다른 케이스 집합의 차**가 된다. 페어링을 강제한다.
> - ⚠️ **바닥 효과 경고.** M26 이 두 arm 모두 바닥에 깔리면 ΔM26 의 부호를 결정할 수 없고,
>   §2 L1-7B 의 성공 기준 ①("M33 의 부호를 결정할 것")이 그대로 무너진다. M26 이 실제로
...
| **L1-7A 1차 실행** | ✅ | [`l1-7-results.md`](l1-7-results.md) — 34케이스 × 3반복 = **102 호출**, 폴백 0건. **H1-7A 는 부분적으로만 지지된다**: 세션 길이는 원안이 이미 지키고(macro 0.873) 대기 단계는 0/1342 인데, **분량은 85/102 가 예산 미달**(초과는 2건)이다. ⚠️ 1차 시도는 `review_feedback` 누락으로 **34호출 전부 프로덕션이 안 내는 프롬프트**로 돌아 폐기했다(그 문서 §5) |
| M20·M21·M22 | ⬜ | 배치(스케줄러) 결과가 필요하다. `busy_saturated` 블록이 그걸 위해 있다 |
...
| **M26 통과 조건 등록** | 🟡 **부분** | §5 「M26 통과 조건」 — **유도 6 · 선택 2 · 미정 1**. ⚠️ **초판(같은 날)을 독립 검증 후 대폭 정정**했다: M22 가 `horizon_coverage_notice` 를 오용해 **의도된 설계 동작을 결함으로 세고 있었고**(골든셋 23/34 가 품질과 무관하게 실패 확정), M20 은 `_CADENCE_OK_RATIO=0.8`(1차 실행의 결함으로 지목했던 그 숫자)을 상속했으며, M18 의 '입도 유도' 는 밴드가 **1.79~6.25%(3.5배 차)** 로 드러나 무너졌다. **M18 은 미정으로 두고 팀 결정을 기다린다.** `tests/test_m26_pass_criteria.py` 21건 |
| **M26 산출** | ⬜ | 두 가지가 막는다 — ① L1-7A **v2 재실행**(현재 M18 은 v1 슬롯 산출) ② **스케줄러 경로**(M20·M21·M22 미측정). 둘 다 되기 전에는 **부분 AND 를 M26 이라 부르지 않는다** |
...
| M33·M34 (F23) | ⬜ | 비교할 M26 통과 조건을 **사전 고정한 뒤** ④층 제거 / 상세 피드백 / 빈 피드백 재시도 3-arm |
| M26 | ⬜ | **산출하지 않는다** — M18·M19·M24·M25 가 비율인데 "통과" 임계값을 §191 이 사전 고정하지 않기로 했다. 분석 시점에 정하면 §0.1 규칙 1번 위반이다 |
