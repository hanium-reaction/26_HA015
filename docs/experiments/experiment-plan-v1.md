...
> | 주지표 | 막는 것 | 상태 |
> |---|---|---|
> | **M26-core** | **M20·M21·M22 미측정** — 스케줄러 경로가 하네스에 없다 | ⬜ **유일한 blocker** |
> | **M18** | 없음 — 임계값 없이 분포로 보고하면 **지금 나온다** | ✅ [`l1-7-results.md`](l1-7-results.md) §2.1 |
>
> ✅ **초판이 blocker 로 적었던 두 개는 해소됐다.** ① L1-7A v2 재실행은 2026-09-03 완료
> (#417) ② M23 분모 0 은 **blocker 가 아니라 N/A 처리 대상**이었다(위 표). ③ M18 임계값은
> M26-core 에서 뺐으므로 더 이상 M26-core 를 막지 않는다.
>
> **M20·M21·M22 가 채워지기 전에는 M26-core 를 내지 않는다.** 현재 하네스가 계산하는 것은
...
| **M26-core 산출** | ⬜ | **유일한 blocker = M20·M21·M22 미측정**(스케줄러 경로 부재). v2 재실행(#417)과 M23 N/A 처리로 나머지는 해소됐다 |
