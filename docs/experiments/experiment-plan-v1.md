...
| **성공 기준** | ① `grounded_clean` 의 M13 ≥ 0.60 ② `omission_probe` 의 M14 가 `no_material` 대비 **유의하게 낮음**(같은 자료 6종 쌍 비교) ③ `unfetchable` 의 M16 = 1.0(못 연 자료를 아는 척하지 않음) |
...
| **M16** | `unfetchable_reask_rate` | `unfetchable` 케이스 | 자료 되묻기가 발생한 케이스 | — | — | 못 연 자료를 아는 척하지 않는가 (#226) |
