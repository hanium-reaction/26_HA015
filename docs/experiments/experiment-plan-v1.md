...
> ### M26 통과 조건 — 2026-09-03 등록
>
> M26 은 M17~M25 의 AND 인데, 그중 몇 개가 비율이라 **"통과" 임계값이 없으면 계산 자체가
> 불가능**했다. 그래서 L1-7A 1차 실행은 M26 을 내지 못했다. 여기서 고정한다.
>
> #### ⚠️ 먼저 — 이것은 완전한 사전등록이 아니다
>
> 이 조건을 쓰는 시점에 **나는 이미 L1-7A 의 M17·M18·M19·M25 수치를 봤다**
> ([`l1-7-results.md`](l1-7-results.md)). 그러므로 §0.1 규칙 1번이 요구하는 "결과를 보기 전"
> 조건을 형식적으로는 못 지켰다. 숨기지 않고 세 가지로 방어한다.
>
> 1. **9개 중 8개는 임계값이 필요 없다.** 아래 표대로 **"이 계획에 위반이 0건인가"** 로
>    자연 이진 판정된다. 고를 여지가 없으므로 관측값에 맞출 방법도 없다.
> 2. **유일한 자유도인 M18 은 데이터가 아니라 시스템 입도에서 유도했다**(아래 유도).
>    L1-7A 의 M18 분포를 다시 열어보지 않고 썼다.
> 3. **M26 을 실제로 계산할 실행은 아직 안 돌았다.** M18 수치는 **v1 슬롯**으로 낸 것이고
>    슬롯 v2 재실행이 남아 있다(그 문서 §8-5). M20·M21·M22 는 스케줄러가 필요해 아직
>    미측정이다. 즉 **M26 이 나올 실행에 대해서는 이 등록이 진짜 사전등록이다.**
>
> #### 항목별 통과 조건
>
> | 지표 | 통과 조건 | 근거 |
> |---|---|---|
> | **M17** | 밴드 밖 leaf **0건** | 사용자에게는 한 건만 어긋나도 어긋난 계획이다(macro 원칙) |
> | **M18** | `abs(총 분 − horizon_minute_budget) ≤ planned_session_min_for / 2` | 아래 유도 |
> | **M19** | `_take_within_budget` 이 잘라낸 leaf **0건** | 〃 |
> | **M20** | `cadence_shortfall_notice` **미발생** | 룰이 이미 부족을 판정한다 — 그 판정을 그대로 쓴다 |
> | **M21** | `_UNPLACED_MARKER` **0건** | 〃 |
> | **M22** | `horizon_coverage_notice` **미발생** | 〃 |
> | **M23** | `missing_milestone_titles` **빈 목록** | 〃 |
> | **M24** | `drop_out_of_cycle_branches` 가 걷어낸 branch **0건** | 〃 |
> | **M25** | `_WAITING_TITLE_RE` 백스톱이 잡은 leaf **0건** | 〃 |
>
> **M20~M25 의 조건을 내가 새로 정하지 않았다는 점이 중요하다.** 전부 **프로덕션이 이미
> 갖고 있는 판정 함수**를 그대로 쓴다. 임계값을 내가 고르면 그 순간 자의가 들어간다.
>
> #### M18 임계값의 유도 — 관측이 아니라 입도에서
>
> 계획은 **세션 단위로만** 만들어진다. 세션 절반짜리를 넣을 수 없다. 따라서 총 분량이
> 예산에서 **반 세션 이내**로 벗어나 있으면, **세션을 하나 더하든 빼든 예산에서 더
> 멀어진다** — 그 계획은 whole-session 계획이 도달할 수 있는 최선이다.
>
> ```
> |총 분 − 예산| ≤ session/2   ⟺   세션 증감으로 더 가까워질 수 없다
> ```
>
> 이 값은 **데이터를 한 번도 보지 않고** 정해진다. "±10%" 같은 비율 밴드를 쓰지 않은 이유가
> 그것이다 — 어떤 비율을 골라도 근거가 관측값밖에 없다.
>
> ⚠️ **양방향이다.** §5 의 M18 정의대로 초과(과잉 생성)와 미달(과소 생성) 모두 실패다.
>
> #### 계산 불가 처리
>
> - **M24 는 `can_refill`(빈도 명시) 케이스에서만 정의된다.** 그 외 케이스에서는 **M26 의
>   AND 에서 빼고**, 뺐다는 사실과 대상 수를 함께 보고한다. 0 을 "통과" 로 세면 M26 이
>   부풀려진다(L1-7A 가 M24 분모에서 42 → 24 로 줄인 것과 같은 처리).
> - **한 지표라도 계산 불가인 케이스는 M26 분모에서 뺀다.** "미측정" 을 "통과" 로 세지 않는다.
>
> #### M26 을 내려면 아직 남은 것
>
> | # | 막는 것 | 왜 |
> |---|---|---|
> | 1 | **L1-7A v2 재실행** | 현재 M18 수치는 v1 슬롯 산출이라 §8-5 가 증거로 쓰지 말라고 적었다 |
> | 2 | **스케줄러 경로를 하네스에** | M20·M21·M22 가 배치 결과를 요구한다. `busy_saturated` 블록이 그걸 위해 있다 |
>
> **둘 다 되기 전에는 M26 을 내지 않는다.** 부분 집합의 AND 를 M26 이라 부르지 않는다 —
> L1-7A 가 M17·M19·M25 동시 통과율(0.853)을 내면서 **"이것은 M26 이 아니다"** 라고 명시한
> 것과 같은 규칙이다.
>
> #### M33 과의 관계
>
> **M33 = ΔM26 이므로 이 등록 없이는 M33 도 못 낸다.** L1-7B 1차 지표가 여기에 걸려 있다.
> ⚠️ 다만 M33 의 두 arm 은 **같은 조건**으로 계산해야 한다 — ④층을 끈 arm 에서도 위 표를
> 그대로 적용한다. arm 마다 다른 기준을 쓰면 차이가 기준 차이인지 층 효과인지 못 가른다.
...
| **M26 통과 조건 등록** | ✅ | §5 「M26 통과 조건」 — 9개 중 **8개는 프로덕션 판정 함수를 그대로 쓴다**(임계값을 내가 안 고른다). 유일한 자유도 M18 은 **반 세션 규칙**으로 입도에서 유도. ⚠️ **완전한 사전등록이 아니다** — L1-7A 수치를 이미 봤다. 다만 M26 이 나올 실행(v2 재실행 + 스케줄러)은 아직 안 돌아 그 실행에 대해서는 진짜 사전등록이다. `tests/test_m26_pass_criteria.py` 15건이 표류를 막는다 |
