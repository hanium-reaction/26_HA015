...
| ⚠️ **결함 설계의 순환성** | 체크리스트를 쓴 사람이 심을 결함도 고르면 "자기가 만든 버그만 잡는 린터"가 되어, **아무도 예상 못한 실패 유형은 영원히 측정되지 않는다.** 완화책 2개를 적용한다: ① 체크리스트 작성자와 **다른 사람이 결함을 설계**(held-out fault design) ② 합성 규칙보다 **실제 인시던트에서 결함 패턴을 채굴** — 우리는 120분 사고 · `_WAITING_TITLE_RE` 백스톱이 생긴 사고(#225) · ADR-0007 범위 이탈 실측이 이미 있어 이 길이 가장 값싸다 |
