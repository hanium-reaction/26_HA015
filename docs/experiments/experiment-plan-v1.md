...
> 층 분업 규칙, 결함 주입 사양, `plan_quality.v4` 출력 계약이 거기 있다. **"결함이란
...
| `plan_quality.v4` | ⬜ | 루브릭 §4 구조화 `findings`. ⚠️ "v3 의 미탐을 고친다" 를 전제로 설계하지 않는다 — **이유·유형·노드 지목을 측정 가능하게** 만드는 것이 목적이고, 그래야 M27b·M27gap·M28a·M28b 가 열린다 |
