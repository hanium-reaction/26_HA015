...
- ⚠️ **아직 닫히지 않은 것**: 저장된 뼈대를 **편집해서** 확정하면 계획은 편집본으로
  만들어지지만 DB 는 옛것을 그대로 둔다. 마일스톤 재조정 HITL(ADR-0007 PR-6)이
  미구현이라 그렇다. 지금은 마일스톤을 읽는 소비자가 없어 무해하지만 진척 롤업(PR-3)이
  들어오면 실제 문제가 된다.

### `GET /goals/{id}/nodes` — milestone 노드가 섞여 나온다(문서 소급)

**동작 변경 없음.** PR-2 가 머지된 시점부터 이미 이렇게 나오고 있었는데 계약에 안 적혀
있었다(응답 예시도 core/subgoal/leaf 세 줄뿐이었다):

- 확정된 마일스톤이 `nodeType="milestone"` · **`parentId=null`** · **`depth=1`** 로 함께
  나온다. 계획 트리와 부모-자식으로 얽지 않기 때문이다 — 얽으면 매 주기 그 트리가 보관될
  때 같이 끌려간다.
- `depth=1` 은 `subgoal` 과 같다. 정렬은 `depth`→`orderIndex` 뿐이라 **주차 목록 사이에
  끼어 나온다.** 트리를 그리는 화면은 `nodeType` 으로 걸러야 한다 — `parentId` 로 그리면
  추가 루트가 되고, depth 들여쓰기로 그리면 주차와 같은 자리에 섞인다.
- `rootNodeId` 는 영향 없다(`depth=0` 인 core 가 항상 먼저 온다).
