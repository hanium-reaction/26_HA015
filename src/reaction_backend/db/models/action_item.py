...
# DB 설계서 v0.7.1 §5.9: v0.7.1 에서 recovery 4 그룹으로 세분화 (recovery_park 추가).
...
class ActionItem(Base, TimestampMixin, SoftDeleteMixin):
    ...
    habit_instance_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("habit_instances.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    # (deprecated for direct lookup) habit_id 는 habit_instance 통해 간접 참조.
    # 단, MVP 편의를 위해 직접 FK 도 유지.
    ...
    # 카테고리 — DB 설계서 v0.7.1 §5.9 (부록 A)
    category: Mapped[str] = mapped_column(
        Enum(*ACTION_CATEGORY_VALUES, name="action_category"),
        nullable=False,
        server_default="other",
    )

    # Morning Brief 카드의 reasonWhyNow / firstStep 에 사용 (우리 개선, ADR §4 보존)
