...
| `runtime.py` | **APScheduler(AsyncIOScheduler) 등록** — `build_scheduler()` 가 8 job 을 KST cron 으로 add_job. job wrapper 가 1회용 세션·repo 를 만들어 sweep/전역 job 호출. |
...
pre_card_notify=종일 */5분 · habit_instances=매일 00:05.
